...
def generate_robot_description_action(context, *args, **kwargs):
    ...
    if model == "r100_a0509":
        urdf_xml = Command([
            ...
            PathJoinSubstitution([
                FindPackageShare("dsr_moveit_config_r100_a0509"),
                "config",
                "r100_a0509.urdf.xacro"
            ]),
            " ",
            "color:=", color,
            " ",
    ...
    # For r100_a0509, use the local ros2_controllers.yaml (no dsr_controller2)
    if model == "r100_a0509":
        ...
        print(f"[INFO] Using r100_a0509 controller.yaml: {adjusted_yaml}")
    elif dynamic_yaml:
...
def control_node_fn(context):
    ...
    if model_value == "r100_a0509":
        controller_yaml = os.path.join(
            get_package_share_directory("dsr_moveit_config_r100_a0509"),
            "config",
            "ros2_controllers.yaml"
        )
        print(f"[INFO] Using r100_a0509 controller.yaml in controller_manager: {controller_yaml}")
...
def generate_launch_description():
    ...
    joint_states_remap = PythonExpression([
        ...
        LaunchConfiguration('model'), "' == 'r100_a0509' and '",
        LaunchConfiguration('name'), "' != '') else ('/joint_states_whole' if '",
        LaunchConfiguration('model'), "' == 'r100_a0509' else ('/' + '",
    ...
    is_r100_a0509_and_whole_enabled = PythonExpression([
        "'", LaunchConfiguration('model'), "' == 'r100_a0509' and '",
        LaunchConfiguration('enable_whole_body_controller'), "'.lower() in ['true','1']"
    ])
    is_r100_a0509_and_whole_disabled = PythonExpression([
        "'", LaunchConfiguration('model'), "' == 'r100_a0509' and '",
    ...
    joint_state_broadcaster_spawner = Node(
        ...
        condition=UnlessCondition(is_r100_a0509),
    )

    # Spawn dsr_controller2 (skip for r100_a0509 to avoid interface conflicts)
    ...
    # Spawners for r100_a0509 (no conditional on the nodes themselves)
    ...
    # For r100_a0509: keep dsr_controller2 available as service/state layer, but do not activate it.
    ...
    # For r100_a0509: start dsr_moveit_controller right after joint_state_broadcaster
    ...
    # For r100_a0509: delay spawners to ensure controller_manager is up
    ...
    delay_whole_body_controller_after_diff_drive = RegisterEventHandler(
        ...
        condition=IfCondition(is_r100_a0509_and_whole_enabled),
    )
    ...
    # For r100_a0509: start MoveGroup + RViz after r100 moveit controller spawner
    delay_rviz_after_r100_moveit_controller = RegisterEventHandler(
        ...
        condition=IfCondition(is_r100_a0509_and_whole_disabled),
    )
