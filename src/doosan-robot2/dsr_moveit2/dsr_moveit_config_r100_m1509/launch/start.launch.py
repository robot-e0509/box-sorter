...
def generate_robot_description_action(context, *args, **kwargs):
    ...
    if model == "r100_m1509":
        ...
        print(f"[INFO] Using r100_m1509 controller.yaml: {adjusted_yaml}")
    elif dynamic_yaml:
...
def control_node_fn(context):
    ...
    if model_value == "r100_m1509":
        controller_yaml = os.path.join(
            get_package_share_directory("dsr_moveit_config_r100_m1509"),
            "config",
            "ros2_controllers.yaml"
        )
        print(f"[INFO] Using r100_m1509 controller.yaml in controller_manager: {controller_yaml}")
...
def generate_launch_description():
    ...
    joint_state_broadcaster_spawner = Node(
        ...
        condition=UnlessCondition(is_r100_m1509),
    )

    # Spawn dsr_controller2 (skip for r100_m1509 to avoid interface conflicts)
    ...
    # Spawners for r100_m1509 (no conditional on the nodes themselves)
    ...
    # For r100_m1509: keep dsr_controller2 available as service/state layer, but do not activate it.
    ...
    # For r100_m1509: start dsr_moveit_controller right after joint_state_broadcaster
    ...
    # For r100_m1509: delay spawners to ensure controller_manager is up
    ...
    # For r100_m1509: start MoveGroup + RViz after r100 moveit controller spawner
