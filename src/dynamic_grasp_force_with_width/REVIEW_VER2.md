...
여기는 그 이후(이동 중 안 떨어뜨리고 안
미끄러지기)만 다룹니다.파지력 계산의 경우에는 ver2와 동일합니다.

[`grasp_v3.ipynb`](grasp_v3.ipynb) 는 [`grasp_v2.ipynb`](grasp_v2.ipynb) 뒤에 이동안정성을 포함했습니다.
...
`weight_calib.json`(무게 캘리브레이션) 없이도 고정 current로 동작하도록 만들어서,이동 로직만 독자적으로 먼저 검증할 수 있습니다.
...
- `release_confirmed()` — release 후 토크가 안 바뀌면 명령이 씹힌 것으로 보고 재전송
...
`PLACE_ZONE_XY`, `resolve_place_pos()`). OCR팀이 실제로 쓰는 값을 반영했습니다.
...
맞출지, 지금 방식을 유지할지"는 여전히 팀 논의가 필요합니다**
...
- **픽업 위치가 실제로 고정인지 가변인지.** OCR팀 코드(`click_and_move.py`)와, 우리 쪽 무게 측정(`_check_cond`, 자세 0.5° 이상 벗어나면 예외)과 구조적으로 충돌할 수 있습니다.
- **박스 폭(width_mm)을 누가 알려주는지.** 
